...
$category_ids = division_category_ids($division);

if (empty($category_ids)) {
    // Division has no matching category_id(s) yet (see category_divisions.php) —
    // show an empty state rather than either an unfiltered or crashing query.
    $all_tickets = [];
} else {
    $placeholders = implode(',', array_fill(0, count($category_ids), '?'));
    $types = str_repeat('i', count($category_ids));

    // No ward_id filter here — a municipal officer sees their division's
    // tickets from every ward, unlike the councillor view which is
    // ward-scoped. w.ward_name assumes a `wards` table with that column;
    // swap/remove the join if your schema differs.
    $sql = "SELECT t.ticket_id AS id, t.title, t.description, t.category_id AS fault_type,
                t.current_status AS status, t.date_created AS created_at, t.ward_id,
...
<head>
<meta charset="UTF-8">
<title>Tickets — M-Unite Municipal Officer View</title>
...
<header class="site-header">
    ...
    <!-- FLAGGED: nav hrefs below point at the councillor page set (I don't
         know your officer-side page filenames). Point these at whatever
         dashboard/reports pages exist for the officer role. -->
    <nav class="navbar">
...
<p class="section-heading" style="margin-top:16px; font-size:0.85rem; color:var(--muted);">
    Division: <strong><?= htmlspecialchars($division) ?></strong>
</p>

<?php
// Renders one ticket card. Adds a ward badge (councillor's version doesn't
// need one since it's already scoped to a single ward; this view spans all
// wards so the ward needs to stay visible).
function render_ticket_card(array $row): void {
    ?>
    <a class="ticket-card" href="ticket_officer.php?id=<?= $row['id'] ?>">
        ...
        <div class="ticket-card-bottom">
            ...
            <span>Ward <?= htmlspecialchars($row['ward_id']) ?></span>
...
<div class="ticket-grid">
    ...
    <?php else: ?>
        <p class="empty-state">No tickets in your division right now.</p>
    <?php endif; ?>
</div>
